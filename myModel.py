...
class MyModel:
	...
	def __get_additional_info_from_trainer__(self):
		...
		if self.model_name.lower() == 'bert':
			...
			for i in range(self.layers):
				self.key_list.append(
					self.trainer.model.encoder.layer[i].attention.self.key.weight.cpu().detach().numpy())
				self.query_list.append(
					self.trainer.model.encoder.layer[i].attention.self.query.weight.cpu().detach().numpy())
		else:
			self.layers = 6
			self.heads = 12
			self.embedding_size = 768
